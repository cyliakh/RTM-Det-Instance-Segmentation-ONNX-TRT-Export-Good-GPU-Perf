...
# --- Force load TensorRT core libraries manually ---
import ctypes
_trt_lib_path = "/home/cylia/w/rtm/rtm/lib/python3.10/site-packages/tensorrt_libs"
for _lib in ["libnvinfer.so.8", "libnvinfer_plugin.so.8", "libnvinfer_builder_resource.so.8.6.1"]:
    _lib_full = os.path.join(_trt_lib_path, _lib)
    if os.path.exists(_lib_full):
        try:
            ctypes.CDLL(_lib_full, mode=ctypes.RTLD_GLOBAL)
        except Exception as e:
            print(f"⚠️ Could not preload {_lib_full}: {e}")
# ---------------------------------------------------
