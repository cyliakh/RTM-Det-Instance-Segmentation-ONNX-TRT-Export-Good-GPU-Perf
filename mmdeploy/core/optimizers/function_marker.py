    ...
    def impl(ys, prefix, level):
        ...
        if isinstance(ys, torch.Tensor):
            if ys not in visit:
                ...
                # ---------------- FIX -----------------
                # Use tensors for shape instead of Python ints
                ys_shape = tuple(s.to(torch.int64) if isinstance(s, torch.Tensor) else torch.tensor(s, dtype=torch.int64) for s in ys.shape)
                # --------------------------------------
