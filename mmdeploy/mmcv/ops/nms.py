...
class ONNXNMSop(torch.autograd.Function):
    """ONNX export placeholder for NMS, fully GPU compatible."""

    @staticmethod
    def forward(ctx, boxes, scores,
                max_output_boxes_per_class, iou_threshold, score_threshold):
        """
        Forward is GPU-only and returns dummy indices for ONNX export.
        """
        batch_size, num_boxes, num_classes = scores.shape
        device = boxes.device

        total_max = min(num_boxes, max_output_boxes_per_class)
        batch_ids = torch.arange(batch_size, device=device).unsqueeze(1).repeat(1, total_max)
        cls_ids = torch.zeros_like(batch_ids)  # dummy class index
        box_ids = torch.arange(total_max, device=device).unsqueeze(0).repeat(batch_size, 1)
        indices = torch.stack([batch_ids, cls_ids, box_ids], dim=-1).view(-1, 3)
        return indices  # [num_selected_indices, 3]

    @staticmethod
    def symbolic(g, boxes, scores, max_output_boxes_per_class, iou_threshold, score_threshold):
        """Export to ONNX NonMaxSuppression, handling symbolic values properly."""
        from torch.onnx import symbolic_helper as sym_help

        # Only wrap Python numbers in Constant tensors
...
class TRTBatchedNMSop(torch.autograd.Function):
    ...
    def symbolic(g,
                ...
                background_label_id: int = -1,
                return_index: bool = False):
        """Symbolic function for mmdeploy::TRTBatchedNMS with safe TopK for TRT."""
        MAX_TOPK = 1024  # TensorRT safe max for TopK

        # clamp TopK values
        pre_topk = min(pre_topk, MAX_TOPK)
        after_topk = min(after_topk, MAX_TOPK)
...
def _multiclass_nms(boxes, scores,
                    ...
                    output_index=False):
    """
    ONNX-friendly multiclass NMS:
    - generates dummy NMS indices for export
    - preserves output shapes like TRT
    """
    scores = scores.permute(0, 2, 1)  # [B, C, N] for ONNXNMSop
    selected_indices = ONNXNMSop.apply(boxes, scores,
                                       max_output_boxes_per_class,
                                       iou_threshold, score_threshold)
    # convert dummy indices to dets and labels shapes (batch, num_det, 5)
    return _select_nms_index(scores, boxes, selected_indices, boxes.shape[0],
                             keep_top_k=keep_top_k, output_index=output_index)
...
def _multiclass_nms_single(boxes: Tensor,
    ...
    if version.parse(torch.__version__) < version.parse('1.13.0'):
        max_output_boxes_per_class = torch.LongTensor(
            [max_output_boxes_per_class])
    score_threshold = torch.as_tensor(score_threshold, dtype=torch.float32, device=boxes.device)
    iou_threshold = torch.as_tensor(iou_threshold, dtype=torch.float32, device=boxes.device)
...
def multiclass_nms__default(
        ...
        """ONNX/TensorRT-friendly multiclass NMS with tensor-based conditional."""

        ctx = FUNCTION_REWRITER.get_context()
        deploy_cfg = ctx.cfg

        batch_size_tensor = torch._shape_as_tensor(boxes)[0]  # keep as tensor
        is_static_batch = torch.tensor(not is_dynamic_batch(deploy_cfg),
                                    dtype=torch.bool,
                                    device=boxes.device)

        # Tensor-based conditional instead of Python boolean
        run_single = is_static_batch & (batch_size_tensor == 1)

        if run_single:  # tensor-based boolean is safe at runtime
...
def multiclass_nms_static(boxes: Tensor,
    ...
    # --- CLAMP TOP-K TO SAFE VALUES FOR TENSORRT ---
    MAX_TOPK = 1024  # safe max for your GPU / TensorRT
    keep_top_k = max_output_boxes_per_class if keep_top_k < 0 else min(max_output_boxes_per_class, keep_top_k, MAX_TOPK)
    if pre_top_k < 0:
        pre_top_k = keep_top_k
    else:
        pre_top_k = min(pre_top_k, MAX_TOPK)
    # -------------------------------------------------
